...
export function ProductRankingCard({ items }: ProductRankingCardProps) {
  return (
    <Card className="dashboard-surface h-full shadow-none">
      ...
      <CardContent className="space-y-8 px-7 pb-8 pt-4">
        {items.map((item, index) => (
          <div key={item.name}>
            <div className="mb-2 flex items-center justify-between text-lg">
              ...
              <span className="font-medium text-primary">${item.value.toLocaleString()}</span>
            </div>
        ...
        {items.length === 0 ? (
          <p className="text-sm text-muted-foreground">No product ranking data yet.</p>
        ) : null}
      </CardContent>
    </Card>
  );
