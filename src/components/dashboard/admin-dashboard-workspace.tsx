...
export function AdminDashboardWorkspace() {
  const [users, setUsers] = useState(adminUserDirectory);
  const [auditLogs, setAuditLogs] = useState(adminAuditLogs);
  const [health, setHealth] = useState(98.2);

  const growthValues = adminGrowthProjection.map((item) => item.value);
  const growthPath = useMemo(() => buildPath(growthValues, 760, 220), [growthValues]);
  const growthAreaPath = `${growthPath} L 760 230 L 0 230 Z`;

  const refreshHealth = () => {
    const value = Number((95 + Math.random() * 4.8).toFixed(1));
    setHealth(value);
    setAuditLogs((prev) => [
      { id: `log-${Date.now()}`, title: "Health Probe Refresh", timeAgo: "Just now" },
      ...prev,
    ].slice(0, 6));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        {adminDashboardMetrics.map((item) => (
      ...
      <section className="grid gap-5 xl:grid-cols-[2fr_1fr]">
        <article className="dashboard-surface border-[#e7e9ee] p-6 shadow-none">
          <div className="mb-4 flex items-start justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-[#667085]">Platform scaling performance (Jan - Jun)</p>
            </div>
          ...
          <div className="h-[250px]">
            <svg viewBox="0 0 760 250" className="h-full w-full" preserveAspectRatio="none">
              ...
              <path d={growthAreaPath} fill="url(#admin-growth-area)" />
              <path d={growthPath} fill="none" stroke="#cca42d" strokeWidth="3.5" strokeLinecap="round" />

              <g fill="#98a2b3" fontSize="12">
                {adminGrowthProjection.map((item, index) => (
                  <text key={item.month} x={(index / Math.max(adminGrowthProjection.length - 1, 1)) * 730 + 10} y="242">
        ...
        <article className="dashboard-surface border-[#e7e9ee] p-6 shadow-none">
            ...
            {users.map((user) => (
              <div key={user.id} className="flex items-center justify-between rounded-xl border border-[#e4e7ec] bg-white px-3 py-3">
                <div className="flex items-center gap-3">
                  <div
                    className="flex h-10 w-10 items-center justify-center rounded-full text-sm font-semibold text-[#344054]"
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`inline-block h-2.5 w-2.5 rounded-full ${
                      user.status === "online" ? "bg-[#a16207]" : "bg-[#d4af37]"
                    }`}
                  />
          ...
          <Button variant="outline" className="mt-4 h-11 w-full rounded-xl text-sm" onClick={() => setUsers(adminUserDirectory)}>
            View All Users
          </Button>
        </article>
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-[1fr_1fr]">
          ...
          <div className="mt-4 space-y-2">
            {auditLogs.map((log) => (
              <div key={log.id} className="flex items-center justify-between rounded-lg border border-[#eef1f4] bg-white px-3 py-3">
                ...
                <span className="text-xs text-[#98a2b3]">{log.timeAgo}</span>
              </div>
            ))}
          </div>
        </article>
        ...
        <article className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#0f172a] via-[#101927] to-[#111827] p-6 shadow-[0_10px_30px_rgba(15,23,42,0.22)]">
          ...
          <p className="mt-2 text-base text-white/75">
            Real-time infrastructure status across availability zones.
          </p>
          ...
          <div className="mt-8">
            ...
            <div className="h-3 rounded-full bg-white/15">
              <div className="h-full rounded-full bg-[#d4af35]" style={{ width: `${health}%` }} />
            </div>
          </div>

          <Button
            className="mt-5 h-10 rounded-full bg-white/10 px-4 text-sm text-white hover:bg-white/20"
